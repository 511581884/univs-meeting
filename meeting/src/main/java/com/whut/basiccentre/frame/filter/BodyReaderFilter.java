package com.whut.basiccentre.frame.filter;
...
/**
 * @Description：由于filter在interceptor之前执行，因此通过filter进行实现，解决请求体中的流只能读取一次的问题。 创建filter，在filter中对request对象用包装后的request替换。
...
public class BodyReaderFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (request instanceof HttpServletRequest) {
            ...
            boolean isProblem = httpRequest.getRequestURL().indexOf("upload") > 0;
